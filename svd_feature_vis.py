...
mat_path = r'D:\Projects\IoT_recognition\20181028\vis\ratio_color.mat'
...
for i in range(X.shape[0]):
    ax.text(xx[i], yy[i], str(y[i] + 1), color=plt.cm.tab20(int(y[i])), fontdict={'size': 8})
...
ax.set_xlim(-edge_max, edge_max)
for i in range(X.shape[0]):
    ax.text(r_x[i], r_y[i], str(y[i] + 1), color=plt.cm.tab20(int(y[i])), fontdict={'size': 8})
